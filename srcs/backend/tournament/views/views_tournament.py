...
from tournament.models import Guest, Tournament, TournamentMatch
    ...
    def post(self, request):
        try:
            ...
            if len(players) != 4:
                return Response(
                    {
                        "error": "Invalid number of players. Must provide exactly 4 players."
                    },
            ...
            for player in players:
                ...
                if user_id:
                    ...
                    return Response(
                        {
                            "error": "Invalid player entry. Each player must have either a user_id or guest_name."
                        },
                        status=HTTP_400_BAD_REQUEST,
                    )
            ...
            with transaction.atomic():
                ...
                TournamentMatch.objects.create(
                    ...
                    match=final_game,
                    position=3,
                    player1_name="FINALIST",
                    player2_name="FINALIST",
                )
                tournament.save()

            return Response(
                {"success": "Tournament created", "tournament_id": tournament.id},
    ...
    if request.method == "POST":
        ...
        try:
            ...
            try:
                ...
                return JsonResponse(
                    {"error": f"Game with id {match_id} not found"}, status=404
                )
            ...
            if match.score_one > match.score_two:
                ...
                winner = (
                    ...
                    if match.player_one
                    else match.player1_guest.display_name
                )
            else:
                ...
                winner = (
                    ...
                    if match.player_two
                    else match.player2_guest.display_name
                )

            # Get all tournament matches
            tournamentMatches = TournamentMatch.objects.filter(
                tournament=tournament
            ).order_by("position")
            next_match = None
            for tournamentMatch in tournamentMatches:
                if tournamentMatch.player1_name == "FINALIST":
                    ...
                    next_match.save()
                    break
                if tournamentMatch.player2_name == "FINALIST":
            ...
            if not next_match:
                ...
                tournament.save()
                return JsonResponse(
                    {"success": "finished", "winner": winner}, status=200
                )
            else:
...
def tournament_list(request):
    tournaments = Tournament.objects.filter(
        Q(registered_users=request.user) | Q(created_by=request.user)
    ).distinct()
    tournament_list = []
    for tournament in tournaments:
        ...
        current_match = None
        if tournament.current_match > 0:
            current_match = TournamentMatch.objects.get(
                tournament=tournament, position=tournament.current_match
            )
        ...
        item = {
            ...
            "current_match": (
                f"{current_match.player1_name} VS {current_match.player2_name}"
                if current_match
                else f"🏆: {tournament.winner}"
            ),
    ...
    def delete(self, request, tournament_id):
        try:
            ...
            if tournament.created_by == request.user:
                tournament.delete()
                return Response(
                    {"success": "Tournament deleted"}, status=status.HTTP_200_OK
                )
            else:
                return Response(
...
class UserOngoingTournamentView(APIView):
    ...
    def get(self, request):
        try:
            ...
            ongoing_tournament = (
                Tournament.objects.filter(created_by=request.user, status="open")
                .order_by("-created_at")
                .first()
            )
            ...
            return Response(
                {"error": "Failed to fetch ongoing tournament."}, status=500
            )
