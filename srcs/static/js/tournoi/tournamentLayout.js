...
export async function displayTournamentLayout(tournamentId) {
  ...
  try {
    ...
    if (response.status === 200 && response.data) {
      ...
      document
        .getElementById("play-next-match")
      ...
      console.warn(
        "Failed to load tournament layout. Status:",
        response.status,
      );
    }
  } catch (error) {
...
function renderBracket(matches) {
  ...
  bracketContainer.innerHTML = matches
    .map(
      (match) => `
          <div class="matchup d-flex justify-content-center align-items-center flex-column gap-1">
...
async function launchNextMatch(tournamentId, data) {
  ...
  const currentMatch = data.matches.find(
    (match) => match.match_id === data.current_match,
  );
  if (!currentMatch)
    return console.error("Current match not found in matches array");

  const payload = { matchId: data.current_match, score_one: 0, score_two: 0 };
  console.log(
    `[launchNextMatch] Launching match ${data.current_match} for tournament ${tournamentId}`,
  );
