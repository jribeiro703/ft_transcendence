...
export async function loadTournamentSetup() {
  ...
  const playerFieldsContainer = document.getElementById(
  ...
  function updatePlayerFields(count) {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      const fieldHTML = playerFieldTemplate.innerHTML
        ...
        .replace(
          /tournament-validation-success/g,
          `tournament-validation-success-${i}`,
        );
  ...
  function updateFieldDependencies() {
    ...
    playerGuestSwitches.forEach((switchElement) => {
      ...
      if (!parent) {
        console.warn(
          "Parent element not found for switchElement:",
          switchElement,
        );
        return;
      }
      ...
      validateButton.addEventListener("click", function () {
        ...
        fetch("/tournament/tokenvalidate", {
          ...
          .then((text) => {
            ...
            console.log(
              "Type of data.valid == true:",
              typeof data.valid == true,
              "Value of data.valid == true:",
              data.valid == true,
            );
  ...
  function validateUniquePlayers() {
    ...
    const playerFields = document.querySelectorAll(".player-select");
    const selectedPlayers = Array.from(playerFields).map(
      (field) => field.value,
    );

    console.log("Selected players:", selectedPlayers);
    // Check for duplicates
    const duplicates = selectedPlayers.filter(
