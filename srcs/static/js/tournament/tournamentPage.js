...
export async function showCreateTournamentForm() {
	...
	document.getElementById('createTournamentForm').addEventListener('submit', async function(event) {
		...
		if (isValid) {
			// const tournamentId = await createTournament(tournamentName);
			//if (tournamentId) {
...
export async function showTournamentView(tournamentName) {
	...
	<div class="container-fluid d-flex flex-column flex-grow-1 p-3">
		<div class="row mb-4">
			<div class="col bg-body border rounded py-1 px-2 d-flex align-items-center">
				<div class="fw-bold ms-3">Tournament Name</div>
				<div class="fw-bold ms-3 text-primary-emphasis h5" id="score-text"><status>${tournamentName}</div>
	...
	document.getElementById('setupTournamentBtn').addEventListener('click', setupTournamentFlow(tournamentName));
